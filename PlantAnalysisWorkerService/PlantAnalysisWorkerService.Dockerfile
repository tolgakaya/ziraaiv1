# Multi-Environment .NET 9.0 Worker Service Dockerfile
...
RUN dotnet build "PlantAnalysisWorkerService.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Publish
FROM build AS publish
ARG BUILD_CONFIGURATION
RUN dotnet publish "PlantAnalysisWorkerService.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
COPY --from=build /src/PlantAnalysisWorkerService/appsettings*.json /app/config/
...
# Create logs directory for worker service
RUN mkdir -p /app/logs/worker && \
    chmod 755 /app/logs/worker

# Create uploads directory for worker service processing
