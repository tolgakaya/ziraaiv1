        ...
        private async Task InitializeRabbitMQAsync()
        {
            ...
            try
            {
                ...
                catch (Exception queueEx) when (queueEx.Message.Contains("PRECONDITION_FAILED"))
                {
                    ...
                    _logger.LogInformation("[RABBITMQ_MULTI_IMAGE_CHANNEL_RECREATE] Recreating channel after PRECONDITION_FAILED");
                    _channel = await _connection.CreateChannelAsync();
                    _logger.LogInformation("[RABBITMQ_MULTI_IMAGE_CHANNEL_RECREATE_SUCCESS] Channel recreated successfully");
                }
